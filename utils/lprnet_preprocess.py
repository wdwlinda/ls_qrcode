...
parser.add_argument("-image", help='image path', default='/storage/user/dwwang/workspace/dataset/CCPD2019/ccpd_base', type=str)
parser.add_argument("-dir_train", help='save directory', default='./data_set/train/lprnet/train', type=str)
parser.add_argument("-dir_val", help='save directory', default='./data_set/train/lprnet/val', type=str)
...
#    img_crop = cv2.resize(img_crop, (94, 24), interpolation=cv2.INTER_LINEAR)
    ...
    if idx % 4 == 0:
        ...
        ret =  cv2.imencode('.jpg', img_crop)[1].tofile(save) #中文名字符问题
        idx_val += 1
    else:
        save = save_dir_train+'/'+lb+suffix      
        # ret =  cv2.imwrite(save, img_crop)
        ret =  cv2.imencode('.jpg', img_crop)[1].tofile(save) #中文名字符问题
        idx_train += 1
