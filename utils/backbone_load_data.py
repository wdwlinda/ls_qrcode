...
class ChaLocDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels_name = img_name.replace('.jpg', '.txt')
        with open(labels_name, 'r') as f:
            labels = f.readline().split(',')
        new_labels = list(map(float, labels))[:4]
        # new_labels = [float(x) for x in labels]
        # new_labels = np.array(labels).astype(float).tolist()
        # print(new_labels)
